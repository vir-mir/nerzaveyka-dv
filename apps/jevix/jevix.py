...
import re
import cgi
from apps.jevix.callback import CallbackTagLs


def valid_text_jevix(text):
    p = Jevix()
    p.cfgAllowTags(['ls','vir-mir','a', 'img', 'i', 'p', 'b', 'u', 's', 'video', 'em',  'strong', 'nobr', 'li', 'ol', 'ul', 'sup', 'abbr', 'sub', 'acronym', 'h4', 'h5', 'h6', 'br', 'hr', 'pre', 'code', 'object', 'param', 'embed', 'blockquote', 'iframe','table','th','tr','td'])

    p.cfgSetTagShort(['br','img', 'hr', 'vir-mir','ls']);

    p.cfgSetTagPreformatted(['pre','code','video'])
    ...
    p.cfgSetTagCallbackFull('ls', CallbackTagLs)

    p.cfgSetXHTMLMode(True)

    p.cfgAllowTagParams('vir-mir', {
        0:'name',
        })
...
# Классы символов генерируются symclass.php
    ...
    def cfgSetTagCallbackFull(self, tag, callback):
        self.tagsRules[tag][self.TR_TAG_CALLBACK_FULL] = callback
    ...
    def parse(self, text, errors = []):
        ...
        if len(self.autoReplace) > 0:
            i = 0
            for k  in  self.autoReplace['from']:
                self.text = self.text.replace(k, self.autoReplace['to'][i])
                i += 1
    ...
    def makeTag(self, tag, params, content, short, parentTag = None):
        ...
        if tagRules.has_key(self.TR_TAG_CALLBACK_FULL):
            text = tagRules[self.TR_TAG_CALLBACK_FULL](tag, resParams, content)
        else:
            text=u'<' + tag
